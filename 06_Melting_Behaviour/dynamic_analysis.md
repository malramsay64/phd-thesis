...
## Comparing fluctuations to the Lennard--Jones model {#sec:fluctuations}

The melting rates of the Trimer at high temperatures
are significantly slower than predicted by the Wilson--Frenkel model,
yet are mostly captured by the fluctuation theory.
To understand more about the fluctuation model
we compare the Trimer to the 2D Lennard--Jones disc.
For the Lennard--Jones simulations
a different order parameter is required,
choosing to use the hexatic order parameter, [@Strandburg1984;@Tobochnik1982]
indicative of bond orientational order
within the liquid and crystal.
The hexatic order parameter $\psi_6$ about a reference particle is given as

$$ \psi_6 = \left|\frac{1}{6} \sum_j^n \exp(i 6 \theta)\right| $$

where $\theta$ is the angle between the vector $(1, 0)$
and the vector from the reference particle to a neighbour.
The hexatic order parameter is calculated using the freud analysis package, [@freud]
and adjusted like such that
...
having no clear value distinguishing the two phases.
...
containing bumps deviating from a Gaussian distribution,
while the Trimer, despite being a more complex molecule,
has a near perfect Gaussian distribution.

Using @eq:probability_liquid and @eq:probability_solid
the distributions of the order parameter
measured within each simulation
can be described through the curvature $\lambda$,
which are tabulated in @tbl:fluctuation_curvature.
These results show how constrained motion is within the Trimer,
the curvature of the trimer liquid is
more than twice that of the LJ crystal.
The curvature of the Trimer in both
the liquid and crystal phases remains
high across the entire temperature range,
which could explain the slower than expected crystal growth.
...
In addition to the slow melting,
the highly rigid structure of the p2 crystal
is likely responsible for the high spinodal point presented in @sec:spinodal.
